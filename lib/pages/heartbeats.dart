...
class _HeartbeatScreenState extends State<HeartbeatScreen>
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: Duration(seconds: 4), // مدة الدورة الكاملة
    );
  ...
  void _showHeartRateDialog() {
    AwesomeDialog(
      ...
      btnCancelText: "Again",
      btnCancelOnPress: () {
        _startAnimation(); // إعادة تشغيل النبضات
      },
      btnOkText: "Ok",
      btnOkOnPress: () {
...
class HeartbeatPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double offsetX = -width * progress; // تحريك النبضات عبر الشاشة

    path.moveTo(offsetX + 0, midY);

    for (double i = 0; i < 3; i++) { // جعلها 3 نبضات بدل 2
      double shift = i * width * 0.6; // تكرار النبضات بدون اختفاء
